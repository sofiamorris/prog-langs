...
(define-type ExprC (U NumC BinopC Symbol AppC IdC))
;;represents a number
...
(define (interp [exp : ExprC][funs : (Listof FundefC)]) : Real
  (match exp 
    [(NumC n) n]
    [(AppC fun args) (cond
                       [(not (= (length args) 1)) (error 'interp "AAQZ more than one arg")]
                       [else
                        (subst (first args) (FundefC-args (find-fun fun funs)) (FundefC-body (find-fun fun funs)))])]
...
(check-equal? (find-fun 'f (list (FundefC 'f '(x y) (BinopC '+ 'x 'y)))) (FundefC 'f '(x y) (BinopC '+ 'x 'y)))
(check-exn #rx"AAQZ" (lambda () (find-fun 'hi (list (FundefC 'f '(x y) (BinopC '+ 'x 'y))))))
...
(define (parse-prog [s : Sexp]) : (Listof FundefC)
  (match s
    [(list (? symbol? r)...) (list (parse-fundef r) (parse-prog (rest s)))]
